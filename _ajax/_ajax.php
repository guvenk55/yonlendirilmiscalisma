...
<?php
  ...
  if(isset($_POST['frm'])){
	...
	break;
	case 'devamedenprojeEkle':
		if(ifo::yetki('1','i')){
					if(isset($_FILES['devamedenprojeler'])){
						$_POST['tarih']=$ifo->suan;
						$_POST['ekleyen']=$_SESSION['id'];
						$r=$ifo->dosyayukle($_FILES['devamedenprojeler'],'../_rsm/projeler','1980x800');
						if($r=='ER'){echo '~danger~Resim yüklenirken bir hata oluştu.'; exit;}
						else if($r=='LEN'){echo '~warning~Resim Boyutu İstenilenden Büyük.'; exit;}
						else if($r=='NA'){echo '~warning~Sadece Resim Yükleyebilirsin.'; exit;}
						else if($r=='MOVE'){echo '~warning~Resim Taşınırken Bir Hata Oluştu'; exit;}
							...
							$_POST['onay']=isset($_POST['onay'])?'1':'0';
							$_POST['resim']=$r[3];
							echo $ifo->form_ekle('devamedenprojeler')?'~success~<h4><i class="fa fa-fw fa-thumbs-up"></i> Başarıyla Yeni Slideri Ekledin.</h4>~_slider':'~warning~<h4><i class="fa fa-fw fa-exclamation-triangle"></i> Bir Hata Oluştu.</h4>Lütfen daha sonra tekrar deneyin.';
						}
					}
					else echo '~warning~<h4><i class="fa fa-fw fa-exclamation-triangle"></i> Resim Seçmedin.</h4>Lütfen bir resim seç.';
				}
	break;
	case 'tamamlananprojeekle':
		if(ifo::yetki('1','i')){
					if(isset($_FILES['tamamlananprojeler'])){
						$_POST['tarih']=$ifo->suan;
						$_POST['ekleyen']=$_SESSION['id'];
						$r=$ifo->dosyayukle($_FILES['tamamlananprojeler'],'../_rsm/projeler','1980x800');
						if($r=='ER'){echo '~danger~Resim yüklenirken bir hata oluştu.'; exit;}
						else if($r=='LEN'){echo '~warning~Resim Boyutu İstenilenden Büyük.'; exit;}
						else if($r=='NA'){echo '~warning~Sadece Resim Yükleyebilirsin.'; exit;}
						else if($r=='MOVE'){echo '~warning~Resim Taşınırken Bir Hata Oluştu'; exit;}
							...
							$_POST['onay']=isset($_POST['onay'])?'1':'0';
							$_POST['resim']=$r[3];
							echo $ifo->form_ekle('tamamlananprojeler')?'~success~<h4><i class="fa fa-fw fa-thumbs-up"></i> Başarıyla Yeni Slideri Ekledin.</h4>~_slider':'~warning~<h4><i class="fa fa-fw fa-exclamation-triangle"></i> Bir Hata Oluştu.</h4>Lütfen daha sonra tekrar deneyin.';
						}
					}
					else echo '~warning~<h4><i class="fa fa-fw fa-exclamation-triangle"></i> Resim Seçmedin.</h4>Lütfen bir resim seç.';
				}
